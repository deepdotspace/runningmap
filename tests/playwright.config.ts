...
export default defineConfig({
  ...
  timeout: 30_000,
  // Saved-route e2e talks to a real backend whose real-time delivery of a
  // just-written record can lag a beat under load. Retry to absorb that
  // eventual-consistency flake (passing tests still run once).
  retries: 2,
  use: {
    baseURL: BASE_URL,
    headless: true,
